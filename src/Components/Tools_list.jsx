...
import { GET_ALL_TOOLS } from "../lib/Query";
import {
  Box,
  Card,
  CardContent,
...
const PricingChip = styled(Chip)(({ theme, pricing }) => {
  const getColor = () => {
    if (pricing?.toLowerCase().includes('free')) return theme.palette.success.main;
    if (pricing?.toLowerCase().includes('premium')) return theme.palette.warning.main;
    return theme.palette.info.main;
  };
  
  return {
    background: `linear-gradient(45deg, ${alpha(getColor(), 0.1)}, ${alpha(getColor(), 0.2)})`,
    color: getColor(),
...
const ToolCard = ({ tool, index }) => {
  ...
  const getToolIcon = (category) => {
    ...
    switch (category?.toLowerCase()) {
      case 'ai': return <HiSparkles {...iconProps} />;
      case 'analytics': return <FiTrendingUp {...iconProps} />;
      case 'productivity': return <FiZap {...iconProps} />;
      default: return <HiSparkles {...iconProps} />;
    }
  };
